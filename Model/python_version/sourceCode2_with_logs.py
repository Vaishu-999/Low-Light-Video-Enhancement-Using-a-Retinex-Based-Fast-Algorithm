...
class RBFAlgorithm:
	...
	def __init__(self,imgpath):
		...
		# Print the frequency (count) of intensity value 128
		print(f"V[4] = {self.vGrayFreq[253]}")
		print(f"S[4] = {self.svGrayFreq[253]}")
	...
	def getVmax(self):
		for pixel_value in range(255,-1,-1):
			if(self.vGrayFreq[pixel_value]!=0):
				print(f"Vmax value : {pixel_value}")
				return pixel_value
		return 0
	...
	def getGamma(self):
		...
		print(f"enlarged c : {c1}")
		cdf=self.getCDF()
		w=0.48
		ansGamma=w*(c1)+(1-w)*cdf
		print(f"gamma : {ansGamma}")
		return ansGamma
	...
	def applyVChannelEnhancing(self):
		...
		# linear enhancement --------------------------------------------------------
		print(f"[Python] Original V value at (100, 100): {self.hsvImg[3][4][2]}")
		print(f"[Python] VmaxValue: {vmaxValue}")
		...
		print(f"[Python] Enhanced V value at (100, 100): {self.enhancedVChannelImg[3][4]}")
		...
		for i in range(self.Rows):
			for j in range(self.Columns):
				if(True):
					...
					if i < 2 and j < 5:
						k=oldvalue*255
						print(f"[PYTHON] Pixel ({i},{j}): old={k}, norm={oldvalue:.4f}, pow={temp:.4f}, gamma={gamma:.4f}, final={temp}")
					...
					if(change1!=0):
						...
						# Log statement
					if (i>870 and i<881) and (j > 1310 and j<1321):
						k = self.enhancedVChannelImg[i][j]
						print(f"after chnage2- change1 :[PYTHON] Pixel ({i},{j}): enhanced={k} ")
					...
					if(self.enhancedVChannelImg[i][j]<100):
						...
						if (i > 870 and i < 881) and (j > 1310 and j < 1321):
							k2 = self.enhancedVChannelImg[i][j]
							print(f"after DR :[PYTHON] Pixel ({i},{j}): enhanced={k2} ")
	...
	def getVES(self):
		...
		smean=self.getSMean()
		#print("smean",smean);
		print(f"ve_mean-smean : {ve_mean-smean}")
		return ve_mean-smean

	print("\nstart Schannel enhancement............................................................\n ")
	def applySChannelEnhancing(self):
		...
		for i in range(self.Rows):
			for j in range(self.Columns):
				...
				if(i==2 and j==2):
					print(f"Before tempvalue : {tempvalue}")
				#ves = ves / 255
				#tempPow = 1 + math.pow(-1, 2 - n) * (math.pow(abs(ves), 2) + abs(ves))
				if i<2 and j<5:
					print(f"tempPow : ", tempPow)

				tempvalue=math.pow(tempvalue,tempPow)
				if (i == 2 and j == 2):
					print(f"after tempvalue : {tempvalue}")
				tempvalue=int(tempvalue*255)
				if (i == 2 and j == 2):
					print(f"after 255 tempvalue : {tempvalue}")
...
obj1.Enhance()
#obj1.showOutput()
obj1.saveOutput('OUTPUTS//input1.jpg')
